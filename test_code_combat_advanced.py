#!/usr/bin/env python3
"""Test avanzati per Code Combat Parte 2 â€” incapsulamento e visibilitÃ .

Coprono i casi limite e le proprietÃ  critiche del refactoring:
...
def test_assignment_trap_runner_integrity():
    ...
    assert r.get_integrity() == 50, (
        ...
        "Se questo test fallisce, l'attributo non Ã¨ realmente privato (manca il "
        "doppio underscore davanti, o c'Ã¨ un setter pubblico che non dovrebbe esserci)."
    )
...
def test_set_min_damage_preserves_invariant():
    """min_damage non puÃ² superare max_damage (invariante)."""
    ice = Icebreaker("Decoder", 3, 7, "decoder")
...
def test_set_max_damage_preserves_invariant():
    """max_damage non puÃ² scendere sotto min_damage."""
...
def test_icebreaker_name_is_read_only():
    ...
    assert not hasattr(ice, "set_name"), (
        "Icebreaker.set_name NON deve esistere: il nome Ã¨ read-only dopo la creazione."
    )
...
def test_icebreaker_type_is_read_only():
    ...
    assert not hasattr(ice, "set_type"), (
        "Icebreaker.set_type NON deve esistere: il tipo Ã¨ read-only dopo la creazione."
    )
...
def test_take_damage_multiple_calls_cumulative():
    """PiÃ¹ chiamate consecutive di take_damage si sommano correttamente."""
...
def test_two_runners_have_independent_state():
    ...
    assert m.get_integrity() == 45, (
        ...
        "Se modificare 'a' tocca 'm', l'attributo Ã¨ stato definito come "
